...
export default function SlowQueryExecutionPlan({ locale }: { locale: string }) {
  const e = locale === "en";
  return (
    <svg width="100%" viewBox="0 0 680 740" className="w-full">
      <defs>
        <marker id="arrow-sq" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
      ...
      <text x="175" y="38" textAnchor="middle" dominantBaseline="central" fontSize="14" fontWeight="500" fill="#A32D2D">{e ? "Before Optimization" : "Before — 개선 전"}</text>

      <rect x="370" y="20" width="270" height="36" rx="8" fill="#E1F5EE" stroke="#1D9E75" strokeWidth="0.5" />
      <text x="505" y="38" textAnchor="middle" dominantBaseline="central" fontSize="14" fontWeight="500" fill="#085041">{e ? "After Optimization" : "After — 개선 후"}</text>

      {/* ── 중앙 구분선 ── */}
      <line x1="340" y1="60" x2="340" y2="620" stroke="#B4B2A9" strokeWidth="0.5" strokeDasharray="4 3" />

      {/* ── PATTERN 1 ── */}
      <text x="340" y="90" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#888780">{e ? "Pattern 1 · Subquery Missing Condition → Full Scan" : "패턴 1 · 서브쿼리 조건 부족 → 풀스캔"}</text>
      ...
      <text x="175" y="148" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#5F5E5A">{e ? "No WHERE Condition" : "WHERE 조건 없음"}</text>
      ...
      <text x="175" y="212" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#791F1F">{e ? "Full Table Scan" : "전체 테이블 풀스캔"}</text>
      ...
      <text x="505" y="148" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#5F5E5A">{e ? "WHERE Condition Added" : "WHERE 조건 추가"}</text>
      ...
      <text x="505" y="212" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#0F6E56">{e ? "Selective Range Lookup" : "필요한 범위만 조회"}</text>
      ...
      {/* ── PATTERN 2 ── */}
      <text x="340" y="312" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#888780">{e ? "Pattern 2 · No Index on Column" : "패턴 2 · 서브쿼리 컬럼 인덱스 없음"}</text>
      ...
      <text x="175" y="370" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#5F5E5A">{e ? "Filter on Unindexed Column" : "인덱스 없는 컬럼 조건"}</text>
      ...
      <text x="505" y="370" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#5F5E5A">{e ? "Filter with Index Created" : "인덱스 생성 후 조건"}</text>
      ...
      {/* ── PATTERN 3 ── */}
      <text x="340" y="476" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#888780">{e ? "Pattern 3 · Insufficient JOIN Columns → Large Hash Join" : "패턴 3 · JOIN 컬럼 부족 → 대용량 Hash Join"}</text>
      ...
      <text x="175" y="534" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#791F1F">{e ? "Insufficient Columns → Large Hash" : "컬럼 부족 → 대용량 해시"}</text>
      ...
      <text x="505" y="534" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#0F6E56">{e ? "Columns Added → Small Join" : "컬럼 추가 → 소량 조인"}</text>
      ...
      <text x="178" y="644" dominantBaseline="central" fontSize="12" fill="#888780">{e ? "Inefficient Path" : "비효율 실행 경로"}</text>
      <rect x="340" y="638" width="12" height="12" rx="3" fill="#E1F5EE" stroke="#1D9E75" strokeWidth="0.5" />
      <text x="358" y="644" dominantBaseline="central" fontSize="12" fill="#888780">{e ? "Optimized Path" : "최적화된 실행 경로"}</text>
    </svg>
  );
}
