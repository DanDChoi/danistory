...
export default function MdReviewArchitecture({ locale }: { locale: string }) {
  const e = locale === "en";
  return (
    <svg width="100%" viewBox="0 0 680 380" className="w-full">
      ...
      <text x="36" y="34" fontSize="11" fill="#888780" fontFamily="monospace">{e ? "WIZWID MD Review System" : "WIZWID MD 상품평 시스템"}</text>

      {/* ── BackOffice ── */}
      <rect x="40" y="46" width="176" height="80" rx="10" fill="#EEEDFE" stroke="#7F77DD" strokeWidth="0.5"/>
      <text x="128" y="66" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#3C3489">{e ? "BackOffice (Admin)" : "BackOffice (관리자)"}</text>
      ...
      <text x="550" y="64" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#633806">{e ? "S3 Temp Bucket" : "S3 임시 버킷"}</text>

      {/* ── S3 메인 버킷 ── */}
      <rect x="464" y="90" width="172" height="36" rx="8" fill="#E1F5EE" stroke="#1D9E75" strokeWidth="0.5"/>
      <text x="550" y="108" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#085041">{e ? "S3 Main Bucket" : "S3 메인 버킷"}</text>
      ...
      <text x="608" y="87" fontSize="10" fill="#E24B4A">{e ? "Delete after save" : "등록 후 삭제"}</text>
      ...
      <text x="340" y="211" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#042C53">{e ? "Review · Image URL" : "상품평 · 이미지 URL"}</text>
      ...
      <text x="128" y="186" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#085041">{e ? "Display Module" : "전시 모듈"}</text>
      <rect x="56" y="198" width="144" height="26" rx="6" fill="#5DCAA5" stroke="none"/>
      <text x="128" y="211" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#04342C">{e ? "Frontend Review Display" : "프론트 상품평 노출"}</text>

      {/* API → 전시 모듈 */}
      <line x1="252" y1="194" x2="216" y2="194" stroke="#1D9E75" strokeWidth="1.5" markerEnd="url(#arr-arch)"/>
      <text x="234" y="186" textAnchor="middle" fontSize="10" fill="#5F5E5A">{e ? "sync" : "연동"}</text>
      ...
      <text x="58"  y="278" dominantBaseline="central" fontSize="11" fill="#888780">{e ? "Spring / jQuery Layer" : "Spring / jQuery 레이어"}</text>

      <rect x="210" y="272" width="12" height="12" rx="3" fill="#FAEEDA" stroke="#BA7517" strokeWidth="0.5"/>
      <text x="228" y="278" dominantBaseline="central" fontSize="11" fill="#888780">{e ? "S3 Temp (volatile)" : "S3 임시 (휘발)"}</text>

      <rect x="360" y="272" width="12" height="12" rx="3" fill="#E1F5EE" stroke="#1D9E75" strokeWidth="0.5"/>
      <text x="378" y="278" dominantBaseline="central" fontSize="11" fill="#888780">{e ? "S3 Main / Display" : "S3 메인 / 전시"}</text>
