...
export default function BoPoStateDiagram({ locale }: { locale: string }) {
  const e = locale === "en";
  return (
    <svg width="100%" viewBox="0 0 680 370" className="w-full">
      ...
      <text x="170" y="74" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#633806">{e ? "Application" : "입점 신청"}</text>
      ...
      <text x="258" y="70" textAnchor="middle" fontSize="10" fill="#1D9E75">{e ? "Approved" : "승인"}</text>

      {/* 입점 신청 → 반려 */}
      <line x1="170" y1="102" x2="170" y2="230" stroke="#E24B4A" strokeWidth="1.5" markerEnd="url(#arr-st)"/>
      <text x="152" y="170" textAnchor="middle" fontSize="10" fill="#E24B4A">{e ? "Rejected" : "반려"}</text>

      {/* 운영 중 */}
      <rect x="288" y="58" width="130" height="44" rx="8" fill="#E1F5EE" stroke="#1D9E75" strokeWidth="0.5"/>
      <text x="353" y="74" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#085041">{e ? "Active" : "운영 중"}</text>
      ...
      <text x="446" y="70" textAnchor="middle" fontSize="10" fill="#BA7517">{e ? "Contract D-30" : "계약 만료일 D-30"}</text>

      {/* 만료 임박 */}
      <rect x="476" y="58" width="130" height="44" rx="8" fill="#FAEEDA" stroke="#BA7517" strokeWidth="0.5"/>
      <text x="541" y="74" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#633806">{e ? "Expiring Soon" : "만료 임박"}</text>
      ...
      <text x="447" y="22" textAnchor="middle" fontSize="10" fill="#1D9E75">{e ? "Renewed" : "재계약"}</text>

      {/* 만료 임박 → 계약 만료 */}
      <line x1="541" y1="102" x2="541" y2="148" stroke="#E24B4A" strokeWidth="1.5" markerEnd="url(#arr-st)"/>
      <text x="560" y="128" fontSize="10" fill="#E24B4A">{e ? "Expiration Passed" : "만료일 경과"}</text>

      {/* 운영 중 → 자진 퇴점 */}
      <line x1="353" y1="102" x2="353" y2="148" stroke="#888780" strokeWidth="1.5" markerEnd="url(#arr-st)"/>
      <text x="370" y="128" fontSize="10" fill="#888780">{e ? "Voluntary Exit" : "자진 퇴점"}</text>

      {/* 계약 만료 */}
      <rect x="476" y="150" width="130" height="44" rx="8" fill="#FCEBEB" stroke="#E24B4A" strokeWidth="0.5"/>
      <text x="541" y="166" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#A32D2D">{e ? "Contract Expired" : "계약 만료"}</text>
      ...
      <text x="353" y="166" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#2C2C2A">{e ? "Offboarding" : "퇴점 처리 중"}</text>
      ...
      <text x="448" y="164" textAnchor="middle" fontSize="10" fill="#E24B4A">{e ? "Offboarding Start" : "퇴점 처리 시작"}</text>

      {/* 반려 */}
      <rect x="110" y="230" width="120" height="44" rx="8" fill="#FCEBEB" stroke="#E24B4A" strokeWidth="0.5"/>
      <text x="170" y="246" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#A32D2D">{e ? "Rejected" : "반려"}</text>
      ...
      <text x="353" y="268" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#2C2C2A">{e ? "Offboarded" : "퇴점 완료"}</text>
      ...
      <text x="370" y="225" fontSize="10" fill="#888780">{e ? "Complete" : "처리 완료"}</text>
      ...
      <text x="96"  y="349" dominantBaseline="central" fontSize="10" fill="#888780">{e ? "In Progress" : "진행 중"}</text>
      <rect x="170" y="344" width="10" height="10" rx="2" fill="#E1F5EE" stroke="#1D9E75" strokeWidth="0.5"/>
      <text x="186" y="349" dominantBaseline="central" fontSize="10" fill="#888780">{e ? "Active" : "정상 운영"}</text>
      <rect x="270" y="344" width="10" height="10" rx="2" fill="#FCEBEB" stroke="#E24B4A" strokeWidth="0.5"/>
      <text x="286" y="349" dominantBaseline="central" fontSize="10" fill="#888780">{e ? "Closed / Rejected" : "종료 / 반려"}</text>
      <rect x="390" y="344" width="10" height="10" rx="2" fill="#F1EFE8" stroke="#B4B2A9" strokeWidth="0.5"/>
      <text x="406" y="349" dominantBaseline="central" fontSize="10" fill="#888780">{e ? "Processing" : "처리 중"}</text>
    </svg>
  );
}
