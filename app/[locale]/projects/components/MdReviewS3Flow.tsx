...
export default function MdReviewS3Flow({ locale }: { locale: string }) {
  const e = locale === "en";
  return (
    <svg width="100%" viewBox="0 0 680 500" className="w-full">
      ...
      <text x="110" y="60" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#3C3489">{e ? "Admin" : "관리자"}</text>
      ...
      <text x="195" y="58" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Select Image" : "이미지 선택"}</text>

      <rect x="222" y="44" width="120" height="44" rx="8" fill="#EEEDFE" stroke="#7F77DD" strokeWidth="0.5"/>
      <text x="282" y="60" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#3C3489">{e ? "Spring Server" : "Spring 서버"}</text>
      ...
      <text x="459" y="60" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#633806">{e ? "S3 Temp Bucket" : "S3 임시 버킷"}</text>
      ...
      <text x="367" y="82" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Return Temp URL" : "임시 URL 반환"}</text>
      ...
      <text x="110" y="146" textAnchor="middle" fontSize="12" fill="#2C2C2A" fontWeight="500">{e ? "Write Review" : "상품평 내용 작성"}</text>
      <text x="110" y="162" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Text + Temp URL" : "텍스트 + 임시 URL 포함"}</text>

      <line x1="170" y1="154" x2="220" y2="154" stroke="#7F77DD" strokeWidth="1.5" markerEnd="url(#arr-md)"/>
      <text x="195" y="146" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Submit" : "등록 요청"}</text>

      <rect x="222" y="132" width="120" height="44" rx="6" fill="#F1EFE8" stroke="#B4B2A9" strokeWidth="0.5"/>
      <text x="282" y="148" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#2C2C2A">{e ? "Validation" : "유효성 검증"}</text>
      <text x="282" y="166" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#5F5E5A">{e ? "Data Parsing" : "데이터 파싱"}</text>
      ...
      <text x="459" y="220" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Temp Bucket File" : "임시 버킷 파일"}</text>
      ...
      <text x="282" y="244" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#5F5E5A">{e ? "Temp → Main Copy" : "임시 → 메인 복사"}</text>
      ...
      <text x="519" y="226" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#085041">{e ? "S3 Main Bucket" : "S3 메인 버킷"}</text>
      ...
      <text x="282" y="308" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#2C2C2A">{e ? "DB Save" : "DB 저장"}</text>
      <text x="282" y="326" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#5F5E5A">{e ? "Main URL + Text" : "메인 URL + 텍스트"}</text>
      ...
      <text x="519" y="326" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#185FA5">{e ? "Review Data" : "상품평 데이터"}</text>
      ...
      <text x="422" y="308" textAnchor="middle" dominantBaseline="central" fontSize="11" fontWeight="500" fill="#A32D2D">{e ? "Temp" : "임시"}</text>
      <text x="422" y="324" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#791F1F">{e ? "Del." : "삭제"}</text>
      ...
      <text x="282" y="390" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#2C2C2A">{e ? "Display Module" : "전시 모듈 연동"}</text>
      <text x="282" y="408" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#5F5E5A">{e ? "Frontend Sync" : "프론트 반영 처리"}</text>

      <rect x="454" y="374" width="130" height="44" rx="8" fill="#E1F5EE" stroke="#1D9E75" strokeWidth="0.5"/>
      <text x="519" y="390" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#085041">{e ? "Frontend Page" : "프론트 페이지"}</text>
      <text x="519" y="408" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#0F6E56">{e ? "Review Display" : "상품평 노출"}</text>
      ...
      <text x="110" y="458" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#085041">{e ? "Complete" : "등록 완료"}</text>
    </svg>
  );
}
