...
export default function WizpaySequenceDiagram({ locale }: { locale: string }) {
  const e = locale === "en";
  return (
    <svg width="100%" viewBox="0 0 680 580" className="w-full">
      ...
      <text x="80"  y="40" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#085041">{e ? "User" : "사용자"}</text>

      <rect x="178" y="20" width="110" height="40" rx="8" fill="#EEEDFE" stroke="#7F77DD" strokeWidth="0.5"/>
      <text x="233" y="33" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#3C3489">{e ? "Order Form" : "주문서"}</text>
      ...
      <text x="391" y="33" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#3C3489">{e ? "Spring Server" : "Spring 서버"}</text>
      ...
      <text x="549" y="33" textAnchor="middle" dominantBaseline="central" fontSize="13" fontWeight="500" fill="#633806">{e ? "Card API" : "카드사 API"}</text>
      ...
      <text x="152" y="84" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Order Entry" : "주문서 진입"}</text>
      ...
      <text x="308" y="114" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Card Info Request" : "카드 정보 조회 요청"}</text>
      ...
      <text x="466" y="144" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "External Card API Call" : "외부 카드사 API 호출"}</text>
      ...
      <text x="466" y="174" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Card List + Policy Response" : "카드 목록 + 정책 응답"}</text>

      {/* 5 - 조건 분기 */}
      <rect x="310" y="196" width="162" height="52" rx="6" fill="#F1EFE8" stroke="#B4B2A9" strokeWidth="0.5"/>
      <text x="391" y="212" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#2C2C2A">{e ? "Conditional Branching" : "조건 분기 처리"}</text>
      <text x="391" y="230" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#5F5E5A">{e ? "Interest-free / Installment / Debit·Credit" : "무이자 / 할부 / 체크·신용"}</text>
      <text x="391" y="244" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#5F5E5A">{e ? "Auto-apply Card Policy" : "카드사별 정책 자동 적용"}</text>
      ...
      <text x="312" y="264" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Send Payment Options" : "분기된 결제 옵션 전달"}</text>

      {/* Dynamic UI */}
      <rect x="153" y="286" width="160" height="40" rx="6" fill="#EEEDFE" stroke="#7F77DD" strokeWidth="0.5"/>
      <text x="233" y="300" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="500" fill="#3C3489">{e ? "Dynamic UI Rendering" : "동적 UI 렌더링"}</text>
      <text x="233" y="316" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#534AB7">{e ? "Auto-build select box" : "select box 자동 구성"}</text>
      ...
      <text x="152" y="344" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Select Payment Method" : "결제 수단 선택"}</text>
      ...
      <text x="308" y="374" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Payment Request (with selection)" : "결제 요청 (선택 정보 포함)"}</text>
      ...
      <text x="466" y="404" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Card Approval Request" : "카드사 승인 요청"}</text>
      ...
      <text x="466" y="434" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Approval Result" : "승인 결과 응답"}</text>
      ...
      <text x="312" y="464" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Payment Complete Response" : "결제 완료 응답"}</text>
      ...
      <text x="152" y="494" textAnchor="middle" fontSize="11" fill="#5F5E5A">{e ? "Show Complete Screen" : "결제 완료 화면 표시"}</text>

      {/* ── Legend ── */}
      <line x1="90"  y1="538" x2="116" y2="538" stroke="#888780" strokeWidth="1.5"/>
      <text x="122" y="542" dominantBaseline="central" fontSize="11" fill="#888780">{e ? "Request" : "요청"}</text>
      <line x1="180" y1="538" x2="206" y2="538" stroke="#888780" strokeWidth="1.5" strokeDasharray="5 3"/>
      <text x="212" y="542" dominantBaseline="central" fontSize="11" fill="#888780">{e ? "Response" : "응답"}</text>
    </svg>
  );
}
