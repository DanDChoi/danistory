...
export default function CovidSweepersArch({ locale }: { locale: string }) {
  const e = locale === "en";
  return (
    <svg width="100%" viewBox="0 0 720 500" className="w-full">
      ...
      <text x="186" y="128" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "JFrame · GameBoard Rendering" : "JFrame · GameBoard 렌더링"}</text>
      <text x="186" y="146" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "Mouse Click Event Handler" : "마우스 클릭 이벤트 처리"}</text>
      <text x="186" y="162" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "Timer · Flag · Mine UI Update" : "타이머 · 깃발 · 지뢰 UI 갱신"}</text>
      ...
      <text x="186" y="242" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "Socket InputStream Listening" : "Socket InputStream 수신 대기"}</text>
      <text x="186" y="260" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "Message → EDT Transfer" : "메시지 수신 → EDT 전달"}</text>
      ...
      <text x="186" y="412" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#1D9E75">{e ? "JTextField + Send Button" : "JTextField + 전송 버튼"}</text>
      ...
      <text x="536" y="128" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "accept() Waiting · Client Connect" : "accept() 대기 · 클라이언트 연결 수락"}</text>
      ...
      <text x="536" y="186" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="600" fill="#A32D2D">{e ? "ClientHandler Thread (1 per client)" : "ClientHandler Thread (1인당 1개)"}</text>
      <text x="536" y="206" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "Dedicated Runnable per Client" : "각 클라이언트 전용 Runnable"}</text>
      <text x="536" y="224" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "Receive → Broadcast to All" : "메시지 수신 → 전체 브로드캐스트"}</text>
      <text x="536" y="242" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#2C2C2A">{e ? "Remove on Disconnect" : "연결 종료 시 목록에서 제거"}</text>
      ...
      <text x="536" y="310" textAnchor="middle" dominantBaseline="central" fontSize="12" fontWeight="600" fill="#633806">{e ? "ClientList (Shared Resource)" : "ClientList (공유 자원)"}</text>
      <text x="536" y="330" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#854F0B">{e ? "List<PrintWriter> Sync Mgmt" : "List<PrintWriter> 동기화 관리"}</text>
      <text x="536" y="344" textAnchor="middle" dominantBaseline="central" fontSize="10" fill="#BA7517">{e ? "thread-safe with synchronized" : "synchronized 블록으로 thread-safe"}</text>
      ...
      <text x="536" y="404" textAnchor="middle" dominantBaseline="central" fontSize="11" fill="#1D9E75">{e ? "Iterate ClientList → Send All" : "ClientList 순회 → 전체 전송"}</text>
      <text x="536" y="418" textAnchor="middle" dominantBaseline="central" fontSize="10" fill="#1D9E75">{e ? "Format: nickname + message" : "닉네임 + 메시지 포맷 전송"}</text>
      ...
      <text x="201" y="491" dominantBaseline="central" fontSize="10" fill="#888780">{e ? "Socket Comm." : "소켓 통신"}</text>
      <rect x="280" y="486" width="10" height="10" rx="2" fill="#FCEBEB" stroke="#E24B4A" strokeWidth="0.5"/>
      <text x="296" y="491" dominantBaseline="central" fontSize="10" fill="#888780">{e ? "Server Thread" : "서버 스레드"}</text>
      <rect x="390" y="486" width="10" height="10" rx="2" fill="#FAEEDA" stroke="#BA7517" strokeWidth="0.5"/>
      <text x="406" y="491" dominantBaseline="central" fontSize="10" fill="#888780">{e ? "Shared Resource" : "공유 자원 (동기화)"}</text>
      <rect x="530" y="486" width="10" height="10" rx="2" fill="#E1F5EE" stroke="#1D9E75" strokeWidth="0.5"/>
      <text x="546" y="491" dominantBaseline="central" fontSize="10" fill="#888780">{e ? "Broadcast" : "브로드캐스트"}</text>
    </svg>
  );
}
